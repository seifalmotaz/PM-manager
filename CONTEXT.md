...
**Task**:
The fundamental unit of work. Lives in a Project. Has status (To Do, In Progress, Done), priority, story points, due date, and optional deadline. Can be assigned to a Sprint. Status transitions are adjacent-only: `To Do → In Progress → Done`. Invalid transitions (e.g., To Do → Done directly) are blocked with an error.
...
- Home view task inclusion — resolved: Home shows all tasks that need attention right now. The rule: (a) any task with status = `in_progress` (regardless of due date), (b) any task with due date = today and status ≠ `done`, and (c) any overdue task (due date < today, status ≠ `done`). Future-dated tasks with status = `todo` do NOT appear on Home — they live in their Project views until their due date arrives or work begins.
...
- Status transitions — resolved: **adjacent only** following standard Scrum. `todo → in_progress → done`. No jumps (e.g., `todo → done` directly). Drag-and-drop in Kanban validates transitions; invalid drops show an error toast.
- Task reopening — resolved: **only during active Sprint**. If a task's Sprint has passed its end date, the task is frozen and cannot be reopened. To work on it again, the PM must create a new task or drag it into a future Sprint (which flags it as `reopened` if the Sprint is active).
- Projectless tasks — resolved: **every workspace gets an auto-created "Inbox" project**. Quick-Add on Home sends tasks to the Inbox of the first selected workspace. Inbox is marked with `isInbox: true` for UI distinction. Users cannot delete their Inbox project.
- Multi-workspace filtering — resolved: **union display**. When multiple workspaces are selected, all tasks appear intermixed in one Kanban board. Each task card shows its project/workspace label. No partitioning or tabs.
- Due date vs. deadline in NLP — resolved: **parsed dates set dueDate**. Deadline is never inferred from NLP; it must be set explicitly via the Task Modal. This preserves the semantic distinction between "target date" (soft) and "hard cutoff" (firm).
- Assignee resolution in NLP — resolved: **case-insensitive, prefer current workspace**. `@Ahmed` matches user "ahmed" regardless of casing. If multiple matches exist across workspaces, prefer the member of the task's workspace. Reject if no workspace member matches.
- Sprint scope in Phase 2 — resolved: **Sprint CRUD + assignment is in scope**. API: `GET/POST /projects/:id/sprints`. Task Modal includes Sprint dropdown. Sprint Board UI is Phase 3.
- Kanban drag implementation — resolved: **use neodrag (Svelte drag library)** instead of custom pointer events. Mobile-compatible, less code, battle-tested. Spec's "pointer events" guidance satisfied via library internals.
- Home page task inclusion — resolved: **separate endpoint `GET /tasks/home`**. The backend applies the domain rule: (a) in_progress tasks, (b) due=today tasks not done, (c) overdue tasks not done. Frontend passes workspace IDs; no client-side merging needed.
- Task creation with NLP — resolved: **two-step flow**. Frontend calls `POST /tasks/parse` first to preview parsed fields, then `POST /tasks` with structured data. Task creation returns only the created task, not parsed fields.
- Task ordering — resolved: **keep `order` column** for future manual reorder. v1 auto-sorts by priority then createdAt; `order` is ignored but exists for v2 when manual reorder is implemented.
